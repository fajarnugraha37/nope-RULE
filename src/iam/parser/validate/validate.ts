...
const allowedPrincipalKeys = new Set([
  "AWS",
  "Service",
  "Federated",
  "CanonicalUser",
...
export function validatePolicySyntax(
  ...
  if (typeof policyDocument !== "object") {
    return [
      {
        path: "",
        message: `Policy must be an object, received type ${typeof policyDocument}`,
      },
    ];
  ...
  if (validationCallbacks.validateVersion) {
    allErrors.push(
      ...validationCallbacks.validateVersion(policyDocument.Version, "")
    );
  } else {
  ...
  allErrors.push(
    ...validateDataTypeIfExists(policyDocument.Id, "Id", "string")
  );
  ...
  if (
    typeof policyDocument.Statement === "object" &&
...
function validateStatement(
  ...
  statementErrors.push(
    ...validateDataTypeIfExists(statement.Sid, `${path}.Sid`, "string")
  );
  ...
  statementErrors.push(
    ...(validationCallbacks.validateStatement?.(statement, path) || [])
  );

  statementErrors.push(
    ...validateOnlyOneOf(statement, path, "Action", "NotAction")
  );
  statementErrors.push(
    ...validateOnlyOneOf(statement, path, "Resource", "NotResource")
  );
  statementErrors.push(
    ...validateOnlyOneOf(statement, path, "Principal", "NotPrincipal")
  );
  ...
  statementErrors.push(
    ...validateActionIfPresent(statement.Action, `${path}.Action`)
  );
  statementErrors.push(
    ...validateActionIfPresent(statement.NotAction, `${path}.NotAction`)
  );
  ...
  statementErrors.push(
    ...validateResource(statement.Resource, `${path}.Resource`)
  );
  statementErrors.push(
    ...validateResource(statement.NotResource, `${path}.NotResource`)
  );

  statementErrors.push(
    ...validateDataTypeIfExists(statement.Principal, `${path}.Principal`, [
      "string",
      "object",
    ])
  );
  statementErrors.push(
    ...validateDataTypeIfExists(
  ...
  statementErrors.push(
    ...validatePrincipal(statement.Principal, `${path}.Principal`)
  );
  statementErrors.push(
    ...validatePrincipal(statement.NotPrincipal, `${path}.NotPrincipal`)
  );

  //TODO: If the condition key exists but there is no value, it is an error
  statementErrors.push(
    ...validateCondition(statement.Condition, `${path}.Condition`)
  );
...
function validatePrincipal(principal: any, path: string): ValidationError[] {
  ...
  if (typeof principal === "object") {
    principalErrors.push(
      ...validateKeys(principal, allowedPrincipalKeys, path)
    );
    principalErrors.push(
      ...validateTypeOrArrayOfTypeIfExists(
        principal.AWS,
...
function validateResource(resource: any, path: string): ValidationError[] {
  ...
  if (typeof resource === "string") {
    ...
    for (let i = 0; i < resource.length; i++) {
      resourceErrors.push(
        ...validateResourceString(resource[i], `${path}[${i}]`)
      );
    }
...
function validateResourceString(
  resourceString: any,
  path: string
): ValidationError[] {
  if (resourceString === "*") {
...
function validateActionString(string: string, path: string): ValidationError[] {
  ...
  if (service && !serviceRegex.test(service)) {
    errors.push({
      path,
  ...
  } else if (action && !actionRegex.test(action)) {
    errors.push({
      path,
...
function validateCondition(condition: any, path: string): ValidationError[] {
  ...
  for (const operator of conditionOperators) {
    ...
    if (splitOperator.length > 2) {
      ...
      if (setOperator && !allowedSetOperators.has(setOperator)) {
        conditionErrors.push({
          path: `${path}.#${operator}`,
...
function validateTypeOrArrayOfTypeIfExists(
  ...
  if (!Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      arrayOfTypeErrors.push(
        ...validateDataTypeIfExists(value[i], `${path}[${i}]`, allowedTypes)
      );
    }
  }
...
function validateDataTypeIfExists(
  ...
  allowedDataTypes = Array.isArray(allowedDataTypes)
    ? allowedDataTypes
    : [allowedDataTypes];
  const errors: ValidationError[] = [];
