import type { ValidationError } from "./validate.interface.js";
...
export function validateResourceControlPolicy(policy: any): ValidationError[] {
  ...
  return validatePolicySyntax(policy, {
    ...
    validateStatement: (statement, path) => {
      ...
      errors.push(
        ...validateAtLeastOneOf(statement, ["Action"], path, policyType)
      );
      errors.push(
        ...validateAtLeastOneOf(
          statement,
...
export function validateEndpointPolicy(policy: any): ValidationError[] {
  return validatePolicySyntax(policy, {
    validateStatement: (statement, path) => {
      ...
      errors.push(
        ...validateAtLeastOneOf(statement, ["Principal"], path, policyType)
      );
...
function validateAtLeastOneOf(
  ...
  let message = `One of ${requiredFields.join(
    " or "
  )} is required in ${policyType}`;
